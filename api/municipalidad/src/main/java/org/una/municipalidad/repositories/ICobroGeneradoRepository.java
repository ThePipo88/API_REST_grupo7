...
public interface ICobroGeneradoRepository extends JpaRepository<CobroGenerado, Long> {
    ...
    @Modifying
    @Query(value = "SELECT u FROM CobroGenerado u LEFT JOIN u.contribuyenteServicio e WHERE e.contribuyente.cedula = :cedula AND e.servicio.tipoServicio = :tipo",nativeQuery = true)
    public List<CobroGenerado> findByCobroCedula(@Param("cedula")String cedula, @Param("tipo")String tipo);
